...
// Declaration of Length template that calculates the number of parameters in a
// tuple
...
// because the specialization above is a better match for tuples with no
// parameters
template <class T, typename... Us> struct Length<tuple<T, Us...>> {
...
template <class Head, typename... Tail> struct TypeAt<tuple<Head, Tail...>, 0> {
  using type = Head;
};

template <class Head, typename... Tail, int i>
struct TypeAt<tuple<Head, Tail...>, i> : public TypeAt<tuple<Tail...>, i - 1> {
};

// Find the index corresponding to a given type
template <class List, class Target> struct IndexOf;

template <class Target> struct IndexOf<tuple<>, Target> {
...
// Count of type in a tuple. Adding for Tuple3 problems. Use TplType to handle
// both std::tuple and Tuple3
...
template <typename X, typename T, template <typename...> class TplType,
          typename... Ts>
struct Count<X, TplType<T, Ts...>>
    : public Count<X, TplType<Ts...>> { // Inherit value
};
...
template <typename A, typename B> struct Replace<tuple<>, A, B> {
  using type = tuple<>;
};
